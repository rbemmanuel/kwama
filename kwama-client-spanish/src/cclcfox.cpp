...
unsigned long kwamaVersion = (((MK_MAJOR_VER << 8)|MK_MINOR_VER) << 8) | MK_RELEASE;
...
setAdminPass(char *pass, int plen)
{
  ...
  FXString path = FXSystem::getHomeDirectory() + "/.kwama";
  FXString fname = "kwama.inf";
...
checkPass(char *pass, int plen)
{
  ...
  pname = FXSystem::getHomeDirectory() + "/.kwama/kwama.inf";
...
{
  if (CCLCFox::cclc)
    fxerror("An instance of kwama Client is already loaded\n");
  active      = FALSE;
  stime       = 0;
  networking  = FALSE;
  client_timeout  = 0;
...
CCLCFox::showMessage(void * message)
{
  FXMessageBox::information(clientwin->getRoot(),
  			MBOX_OK,_("Mensaje del Administrador del Ciber"), (char *)message);
}
...
#endif
  oowed = (double)owed;     
  snprintf(buf,64,"%.2f", oowed/100.00);  
  clientwin->setOwed(buf);
  
}

void
CCLCFox::setProducts(FXuint ppowed)
{
  char buf[64];
  double powed;
#ifdef DEBUG
...
#endif
  powed = (double)ppowed; 
  snprintf(buf,64,"%.2f",powed/100.00);
  clientwin->setProducts(buf);
  
}

void
CCLCFox::setTotal(FXuint ttowed)
{
 char buf[64];
 double towed;
  ...
  snprintf(buf,64,"%.2f", towed/100.00);
  clientwin->setTotal(buf);
...
CCLCFox::onTimer(FXObject*,FXSelector,void*)
{
  ...
  if (active) {
	...
	sprintf(wstr, _("Usted tiene menos de %d minutos para navegar."), wtime/60);
	time_warned = 1;
	dispMessage(wstr);
