...
main(int argc,char *argv[])
{
  ...
  FXApp app("kwama","kwama Ciber Temporizador");
  if (!homedir)
#ifndef WIN32
    FXSystem::setCurrentDirectory(FXSystem::getHomeDirectory() + "/.kwama/");
#else
  if (FXFile::isAbsolute(argv[0])){
    ...
    FXString dirname = FXString(homedir) + "/.kwama-cli";
  ...
  textdomain("kwama");
# ifdef WIN32
  bindtextdomain("kwama","./locale");
# endif
#endif
  init_img("/usr/local/lib/libkwmimg.so","1.0");
